...
async function runtsCode(req, res) {
  const { code, input } = req.body; 

  // Save the user's code to a file
  const userCodeFilePath = path.join(__dirname, "user_code.ts");
  ...
  exec(
    `docker run --rm -v "${resolvedPath}:/usr/src/app" ts-compiler`,
    (error, stdout, stderr) => {
      ...
      try {
        ...
        const compiledJavaScriptPath = path.join(__dirname, "user_code.js");
        if (fs.existsSync(compiledJavaScriptPath)) {
          fs.unlinkSync(compiledJavaScriptPath);
        }
